"""Turn a few words into a structured, mobile-friendly creative brief.

Used in the CLI's ``--prompt`` mode rather than the auto-dispatch registry,
because here the *medium* is a deliberate user choice, not something to be
guessed against arithmetic or unit queries.
"""
...
def normalize_medium(label: Optional[str]) -> Optional[str]:
    if not label:
        return None
...
def detect_medium(text: str) -> Optional[str]:
    lowered = text.lower()
    for medium, aliases in _MEDIUM_SYNONYMS.items():
...
def _shape(seed: str, medium: str) -> Tuple[str, List[str]]:
    profile = _RECIPES[medium]
    cleaned = seed.strip().rstrip(".")
    answer = (
        f"{profile['title']}: {cleaned}. "
...
def build_creative_prompt(seed: str, medium_hint: Optional[str] = None) -> Solution:
    ...
    medium = normalize_medium(medium_hint) or detect_medium(seed) or "art"
    answer, details = _shape(seed, medium)
    return Solution(
        kind="Creative Prompt",
        answer=answer,
        details=details,
        confidence=1.0,
        source="creative",
...
class CreativeSolver(Solver):
    """Registry-compatible wrapper, used by tooling that enumerates solvers."""

    name = "creative"
    kind = "Creative Prompt"
    description = "Build a structured creative brief (photo/video/music/art/poem)."

    def solve(self, problem: str) -> Optional[Solution]:
        if not problem.strip():
            return None
        return build_creative_prompt(problem)
